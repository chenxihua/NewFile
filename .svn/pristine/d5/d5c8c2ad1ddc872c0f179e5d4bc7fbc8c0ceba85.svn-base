...
<body>	
	...
	<script type="text/javascript" id="barDemo">
	  <a class="layui-btn layui-btn-xs" lay-event="reAssignService">再分配</a>
	</script>
	...
	<script>			
  	...
  	table.on('tool(test)', function(obj){
    	...
    	//再分配
    	if(obj.event === 'reAssignService'){
  			layer.open({
  				title: ['再分配','font-size:18px'],
  		  		type: 2, 
		  		area: ['450px', '450px'],
  		  		content: '<%=basePath %>service/toReAssignService.action?id='+data.id ,
                end : function() {
                    table.reload('test',{});
                }
  			});  
