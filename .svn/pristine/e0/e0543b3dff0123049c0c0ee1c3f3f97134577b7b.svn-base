...
<button class="layui-btn" id="newSave" data-type="getSaveData">新增</button>
</div> -->

<table class="layui-table" id="test" lay-filter="test"></table>
<script type="text/html" id="barDemo">
<a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="detail">审核阶段</a>
</script>
...
layui.use(['form','layer','table'], function(){
	  ...
	  testTable.render({
	    elem: '#test'
	    ,url:'saleChance/selectAllStatusZero'
	    ,cols: [[
		...
		testTable.on('tool(test)', function(obj){
		  ...
		  var tr = obj.tr;
		  if(layEvent === 'detail'){
			  showSaleChanceInfos('销售机会详情',data.id);
		  }
		});
		
		//添加按钮触发时间
		$("#newSave").click(function(){
			showSaleChanceInfos("添加销售机会",-1);
        })
		
		function showSaleChanceInfos(title,id){
			if(id>0){
				var url = "pages/sale/editSalePage.jsp";
				url += '?id='+id;
				layer.open({
					type:2,
					title:title,
				...
				var url = "pages/sale/addSalePage.jsp";
				layer.open({
					type:2,
					title:title,
					closeBtn : 1, 
