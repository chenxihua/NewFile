...
layui.use('form', function(){
	...
	form.on('submit(affirmativeDemo)', function(data){
		$.post("<%=basePath %>service/reqForAssignService.action",data.field,function(ret){
			if(ret.msg){
				layer.msg("服务转移成功", {time: 500});	
				//设置为延迟执行				
				setTimeout( function() {
					//先得到当前iframe层的索引
