...
<body>
 ...
 <form class="layui-form" >  
  ...
  <div class="layui-form-item layui-form-text">
    ...
    <div class="layui-input-block">
      <textarea name="handle" class="layui-textarea" lay-verify="required">${service.handle}</textarea>
    </div>
  </div>
...
layui.use('form', function(){
	...
	form.on('submit(formDemo)', function(data){
		$.post("<%=basePath %>service/handleService.action",data.field,function(ret){
			if(ret.msg){
				layer.msg("提交处理成功", {time: 500});	
				//设置为延迟执行				
				setTimeout( function() {
					//先得到当前iframe层的索引
