...
<body>	
	<br>
	<!-- 创建服务按钮, createServiceBtn -->	
	<div class="layui-btn-group demoTable">
  		<button class="layui-btn" id="createServiceBtn">创建服务</button>
	</div>
	...
	<script type="text/javascript" id="barDemo">
	  <a class="layui-btn layui-btn-xs" lay-event="handleService">处理</a>
	  <a class="layui-btn layui-btn-danger layui-btn-xs" lay-event="reqForAssignService">转移</a>
	</script>
	...
	<script>			
		layui.use(['form','layer','table'], function(){
          	...
          	table.render({
            	...
            	,url:'<%=basePath %>/service/handleList'  //数据请求路径
            	,cellMinWidth: 80
            	,cols: [[
  	...
  	table.on('tool(test)', function(obj){
    	...
    	if(obj.event === 'handleService'){
  			layer.open({
				...
				closeBtn : 1, 
  		  		area: ['450px', '520px'],
                shadeClose : false,
  		  		content: '<%=basePath %>service/toHandleService.action?userId='+data.id,
                end : function() {
                    table.reload('test',{});
                }
  			});  
    	...
    	//转移服务
    	if(obj.event === 'reqForAssignService'){
  			layer.open({
  				title: ['转移服务','font-size:18px'],
  		  		type: 2, 
  		  		area: ['450px', '520px'],
  		  		content: '<%=basePath %>service/toReqForAssignService.action?id='+data.id,
                end : function() {
                    table.reload('test',{});
                }
  			});  
